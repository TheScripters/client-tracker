...
<div id="content">
  ...
  <div id="calendar" style="width:530px">
  <table width="100%">
    <tr>
  ...
  <table width="100%">
    ...
    <?php
      $time = time();
      $newTime = mktime(0,0,0,date("n"),$daysInMonth+1,date("Y"));
      $mEventsQuery = mysql_query("SELECT event_time FROM ".$config['db']['prefix']."calendar WHERE event_time > ".$firstOfMonth." AND event_time < ".$newTime);
      $mEventArray = array();
      for ($d=1;$d<=31;$d++) {
        $mEventArray[$d] = 0;
      }
